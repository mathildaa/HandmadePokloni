	...
	else
	{
		...
		if(isset($_POST['dodaj']))
		{
			...
			{	
				...
				$autori = $vezaNaBazu->query("select id from autori where username like '$un'");
				foreach($autori as $a)
				{
					$autor = $a['id'];
				}
				$novosti = $vezaNaBazu->query("select id from novost");
				$id = 1;
				if (!$novosti) {
				...
				$komentar = 0;
				if(isset($_POST['komentari'])) $komentar = 1;
				$id++;
				$dodavanje = $vezaNaBazu->exec("insert into novost(id, naslov, tekst, datum, komentar, slika, autor) values ($id, '$naslov', '$tekst', NOW(), '$komentar', '$slika', $autor)");
				if (!$dodavanje) {
					  $greska = $vezaNaBazu->errorInfo();
					  print "SQL greška: " . $greska[2];
					  exit();
					...
					<div class="news">	
							<form id="forma" method="post">					
									...
									<div id = "desno">
										...
										<input type="checkbox" name="komentari" value="komentari"> Dozvoli komentare<br>
										<div class="desno-cont">
											...
											<input id="izadji" style="height:50px; width:100px" type="submit" name="izadji" value="Nazad">
